...
class Node:
    def __init__(self, x, y, theta, vl, vr, g, h, v_comm, arc_rad, dt_comm,steer_ang, parent=None):
        self.x = x
        self.y = y
...
class A_Star_Planner:
    def __init__(self, start_x, start_y, start_th, goal_x,goal_y, other_args):
        ...
        self.rad = other_args[0]
        self.width = other_args[1]
        self.wheelbase = other_args[2]
        self.wgt_heur = other_args[3]
        self.goal_radius = other_args[4]
        self.th_gain = other_args[5]
        self.map_bounds = other_args[6]
        self.cost_map = other_args[7]
    ...
    def a_star(self):
        ...
        poss_dt = np.arange(.5,6,.25)
        poss_vel = np.array([-1,1])

        #### Initiate Search
        start_node = Node(self.start_x, self.start_y, self.start_th, 0, 0, 0, self.heuristic(self.start_x, self.start_y),0,0,0,0, parent=None)
        ...
        while open_list:
            ...
            if curr.h/self.wgt_heur <= self.goal_radius:
                path = []
                while curr:
                    path.append((curr.x, curr.y, curr.theta, curr.arc_radius, curr.transl_velocity, curr.dt))
                    curr = curr.parent
                self.path = path[::-1]
                return path[::-1] 
            ...
            for arc_rad in poss_R:
                ...
                for dt_comm in poss_dt:
                    for v_comm in poss_vel:
                        ...
                        next_th = curr.theta-ang_velo*dt_comm
                        ...
                        in_bounds = True
                        for angv_step in np.linspace(0,ang_velo, 7):
                        ...
                        # Calculate g, h, and fq
                        g_cost = curr.g + 20*(np.abs(curr.steer_ang-steer_angle))+self.cost_map[math.floor(next_x), math.floor(next_y)]
                        h_cost = self.wgt_heur*self.heuristic(next_x, next_y)
                        neighbor_node = Node(next_x, next_y, next_th, vl, vr, g_cost, h_cost,v_comm, arc_rad,dt_comm,steer_angle, curr)
    ...
    def plot_path(self):
        ...
        plt.imshow(self.cost_map, cmap='viridis', origin='lower', extent=(self.map_bounds[0],self.map_bounds[2],self.map_bounds[1],self.map_bounds[3]))
...
if __name__ == '__main__':
    ...
    start_x, start_y, start_th = 0, 0, 3*np.pi/2   # Start position
    goal_x, goal_y = -4,2     # Goal position
    ...
    while (time.time()+2*last_plan-start <= completion_time):
        ...
        new_planner = A_Star_Planner(start_x, start_y, start_th, goal_x, goal_y, [rad, width, wheelbase, wgt_heur, goal_radius, gain, map_bounds, cost_map]) 
        plan = new_planner.a_star()  # Plan A-star
        print('Time to Complete', time.time()-plan_start, 'seconds with weighted heuristic:', wgt_heur)
