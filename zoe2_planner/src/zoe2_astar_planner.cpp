...
#include <algorithm>
#include <limits>
#include "zoe2_planner/zoe2_astar_planner.h"
...
std::vector<std::tuple<double, double, double, double, double, double, double>> AStarPlanner::a_star(double start_x, double start_y, double start_th, double goal_x, double goal_y) {
    ...
    // Add the negative values of the radii
    std::vector<double> neg_poss_R;
    for (double rad : poss_R) {
        neg_poss_R.push_back(-rad);
    }
    poss_R.insert(poss_R.end(), neg_poss_R.begin(), neg_poss_R.end());
    std::sort(poss_R.begin(), poss_R.end());
